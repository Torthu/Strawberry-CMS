...
define('PATH', backup_directory.'/');
define('URL', 'data/backup/');
...
define('LIMIT', 1);
// mysql сервер
define('DBHOST', $config['dbserver']); //'localhost:3306'
...
/**/
if (straw_get_rights('backup', 'read') and straw_get_rights('backup', 'write')) {
...
$auth = 1;
$error = '';
$user[1] = $config['dbuser'];
$user[2] = $config['dbpassword'];
...
if (!$auth || (isset($_SERVER['QUERY_STRING']) && $_SERVER['QUERY_STRING'] == 'reload')) {
	setcookie("sxd");
	echo tpl_page(tpl_auth($error ? tpl_error($error) : ''), "<SCRIPT>if (jsEnabled) {document.write('<INPUT TYPE=submit VALUE=Логин>');}</SCRIPT>");
...
echo "<html><head><meta http-equiv=\"content-type\" content=\"text/html; charset=".$config['charset']."\"><title>STRAWBERRY 1.2.0 ::: АдминЦентр</title><link href=\"skins/default.css\" rel=\"stylesheet\" type=\"text/css\" media=\"screen\"></head><body>";
echo 'Вам запрещён доступ к этому модулю!';
echo "</body></html>";
...
class dumper {
	function dumper() {
global $config;
		if (file_exists(data_directory."/dumper.php")) {
		    include(data_directory."/dumper.php");
		}
		else{
			$this->SET['last_action'] = 0;
			$this->SET['last_db_backup'] = '';
			$this->SET['tables'] = (!empty($config['dbprefix']) ? $config['dbprefix'].'*' : '');
	...
	function fn_save(){
		if (SC) {
			$ne = !file_exists(data_directory."/dumper.php");
		    $fp = fopen(data_directory."/dumper.php", "wb");
        	fwrite($fp, "<?php\r\n#_strawberry\r\n\tif (!defined(\"str_adm\")) {\r\n\theader(\"Location: ../../index.php\");\r\n\texit;\r\n\t}\r\n\$this->SET = " . fn_arr2str($this->SET) . "\n?>");
        	fclose($fp);
			if ($ne) @chmod(data_directory."/dumper.php", 0666);
			$this->fn_index();
		}
	}
...
function tpl_main(){
global $SK, $config;
$prf = !empty($SK->vars['tables']) ? $SK->vars['tables'] : (!empty($config['dbprefix']) ? $config['dbprefix'].'*' : '');
return <<<HTML
...
<TR>
<TD>Фильтр таблиц:</TD>
<TD><INPUT NAME=tables TYPE=text CLASS=text VALUE='{$prf}'></TD>
</TR>
<TR>
<TD>Метод сжатия:</TD>
